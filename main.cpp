...
void process_and_write(std::queue<std::pair<std::string, std::string>> reads, Align &align)
{
    while (!reads.empty())
    {
...
// std::list<std::future<void>> load_blocks(std::string read_file, thread_pool &threads1, std::list<Align> &aligns, size_t max_block, size_t block_size) // mt
void load_blocks(std::string read_file, std::list<Align> &aligns, size_t max_block, size_t block_size) // st
{
    std::ifstream fin(read_file);
    // std::list<std::future<void>> futures; // mt
    auto iter = aligns.begin();
    do
    {
        std::string name;
        std::string read;
        while (fin >> name >> read)
        {
            ...
            while (reads.size() < block_size && fin >> name >> read)
                reads.emplace(name, read);
            // std::unique_lock lck(mtx); // mt
            // cv.wait(lck, [max_block] // mt
            // { return block_num < max_block; }); // mt
            if (iter == aligns.end())
                iter = aligns.begin();
            // futures.push_back(threads1.submit(std::bind(process_and_write, std::move(reads), std::ref(*iter)))); // mt
            ++iter;
            ++block_num;

            process_and_write(std::move(reads), (*iter)); // st
        }
    } while (fin.good());
    // return futures; // mt
}
...
{
    int n_sz = 5, r_sz = 2, t_sz = 1, max_e_sz = 8, lseqlb = 100, lsequb = 200, gseqlb = 10000, gsequb = 20000, aseqlb = 50, asequb = 100, seq_num = 1, head_in = 10, tail_in = 10;
    ...
    size_t kb = 1024;
    size_t chunk_sz = size_t(128) * kb * kb;
    std::list<Align> aligns;
    std::vector<std::string> mg_files;
    for (int i = 0; i < threads1_sz; ++i)
    {
        mg_files.push_back(read_file + std::to_string(i) + ".mg");
        aligns.emplace_back(argc_r, argv_r, file2seq, file2browheel, chunk_sz, mg_files.back());
    }
    aligns.front().draw("graph.gv");
    ...
    load_blocks(read_file, aligns, max_block, block_size); // st
