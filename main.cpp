...
int main(int argc, char **argv)
{
    ...
    if (vm.count("index"))
    {
        ...
        uint8_t *bwt = SA2bwt(RevRefFile+".sa", RevRefFile+".bwt", 10000, revref);
        RankVec bwtRank(bwt, revref_sz);
        bwtRank.saveRankVec(RevRefFile+".rnk");

        bwtRank.loadRankVec(RevRefFile+".rnk"); // debug
        std::cerr << "check bwt: " << check_bwt(bwt, std::filesystem::file_size(RevRefFile+".bwt")/sizeof(uint8_t), bwtRank, revref) << '\n'; // debug
    ...
    //     fout << int2base[browheel.bwt[i]];
    // fout << '\n';
    // fout.close();

    //////////////////////////////////////////////

    // std::string bwtfile = "test.fa.gap.bwt"
    // uint8_t * bwt = (uint8_t *) malloc(std::filesystem::file_size(bwtfile));
    // std::ifstream fin(bwtfile);
    // fin.read((char *)bwt, std::filesystem::file_size(bwtfile));
    // fin.close();
    // for (int i=0; i<std::filesystem::file_size(bwtfile); ++i)
    //     std::cout << int(bwt[i]);
    // std::cout << '\n';
