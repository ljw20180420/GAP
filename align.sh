...
for round in $(seq 0 $mr); do
	...
	T=$(($Ti + $dT * $round))
	$excu --threads_sz $tz --input $read_file --min_seg_num 1 --max_seg_num $msn --block_size $bs ---nodes --names node0 ---roots node0 ---targets node0 ---globals --names $genome --tails node0 --heads node0 --Ts $T --r "true"
	cd ..
done
mkdir tmplast
cd tmplast
